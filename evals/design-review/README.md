...
- A full run (6 cases × 3 runs × 2 arms) takes about 13 minutes with `-j 4`. The baseline run cost about $17 of API-equivalent tokens, much of it from a `no-phantom-code` grader that sent the whole transcript to the judge. That grader has since been removed. Expect roughly $10 per run, but that isn't measured yet.
...
This is v1.3.0, 2026-09-19 (3 runs × 2 arms, Opus agent, Sonnet judge). Case 04 is from its re-run after its fixture and rubric were fixed. Scores are recomputed from the per-grader results without `no-phantom-code`, which was removed afterwards. Hallucinated code is rare and isn't scored; spot-check by hand.

| Case | With | Without | Δ | Skill fired (with) |
|---|---|---|---|---|
| 01-orders-module | 0.83 | 0.83 | 0.00 | 3/3 |
| 02-pr-diff | 1.00 | 1.00 | 0.00 | 3/3 |
| 03-cluster | 0.83 | 0.50 | **+0.33** | 3/3 |
| 04-clean-go | 1.00 | 0.67 | **+0.33** | 3/3 |
...
- **01 and 02 have no headroom.** Plain Opus already puts a clear boundary leak above naming nits, and both arms missed the ranking once on 01.
