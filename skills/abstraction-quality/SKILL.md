...
description: Evaluates whether abstractions are genuine or merely structural. Use when the user asks to review an abstraction, when an abstraction feels leaky, when layers operate at the same level of abstraction, or when decorator/wrapper patterns add boilerplate without depth. Checks layer boundaries and abstraction quality.
