---
name: code-evolution
description: Evaluates whether changes to existing code maintain or degrade design quality. Use when the user asks to review how code has evolved, when reviewing modifications to existing code, when a change looks patched-on rather than designed-in, or when code has accumulated layers of changes. Checks the "designed this way" standard, repetition, comment maintenance, and technical debt trajectory.
...
**Stale comments are worse than missing comments — they actively mislead.** Once readers discover comments can't be trusted, they stop reading them entirely.
